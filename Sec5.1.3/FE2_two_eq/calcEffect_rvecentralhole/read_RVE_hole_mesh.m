...
function [nnode,coords,connect,bound] = read_RVE_hole_mesh(infile,elemType)
% 
%   clear;clc;
...
% 
  ...
  if ( string(phyname{end,2}) == '"matrix"' )
      connect_mat = connect((connect(:,1) == phyname{end,1}), 3:2+nelnodes); % SPE 
  else
...
%
  if ( string(phyname{end-4,2}) == '"left"' )
      bound.left = connect((connect(:,1) == phyname{end-4,1}),3:4);
  else
      fprintf('SPE left boundary physical number not found!');
  end 
  if ( string(phyname{end-3,2}) == '"right"' )
      bound.right = connect((connect(:,1) == phyname{end-3,1}),3:4);
  else
      fprintf('SPE right boundary physical number not found!');
  end 
  if ( string(phyname{end-2,2}) == '"lower"' )
      bound.lower = connect((connect(:,1) == phyname{end-2,1}),3:4);
  else
      fprintf('SPE right boundary physical number not found!');
  end  
  if ( string(phyname{end-1,2}) == '"upper"' )
      bound.upper = connect((connect(:,1) == phyname{end-1,1}),3:4);
  else
      fprintf('SPE right boundary physical number not found!');
  end 
%  corner nodes
