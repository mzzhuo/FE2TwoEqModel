function [dFlux_mat,flux_mat,sour_mat,dSou_mat,sour_inc,dSou_inc] = micro(gradU_mat,gradU_inc,u_mat,u_inc)
% 
...
delta = delta(nnode+1:nnode+nlamb+3,:);
rhs = [zeros(ndof*nnode,4); -C];
solu = K \ rhs;
% stiff = K1'*delta*solu;
stiff = [K_peri; K_corn]'*delta*solu;
% 
D_b  = coords(bounodes,:);
cols = dofArray(bounodes,1);
