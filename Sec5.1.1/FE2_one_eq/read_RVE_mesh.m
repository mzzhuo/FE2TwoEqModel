...
function [nnode,coords,connect,elemTag,bound] = read_RVE_mesh(infile,elemType)
% 
%   clear;clc;
...
% 
  ...
  if ( string(phyname{6,2}) == '"matrix"' )
      connect_mat = connect((connect(:,1) == phyname{6,1}), 3:2+nelnodes); % SPE 
  else
      fprintf('SPE physical number not found!');
  end
  if ( string(phyname{7,2}) == '"inclusion"' )
      connect_inc = connect((connect(:,1) == phyname{7,1}), 3:2+nelnodes); % SPE 
  else
      fprintf('SPE physical number not found!');
  end
...
%
  if ( string(phyname{2,2}) == '"left"' )
      bound.left = connect((connect(:,1) == phyname{2,1}),3:4);
  else
      fprintf('SPE left boundary physical number not found!');
  end 
  if ( string(phyname{3,2}) == '"right"' )
      bound.right = connect((connect(:,1) == phyname{3,1}),3:4);
  else
      fprintf('SPE right boundary physical number not found!');
  end 
  if ( string(phyname{4,2}) == '"lower"' )
      bound.lower = connect((connect(:,1) == phyname{4,1}),3:4);
  else
      fprintf('SPE right boundary physical number not found!');
  end  
  if ( string(phyname{5,2}) == '"upper"' )
      bound.upper = connect((connect(:,1) == phyname{5,1}),3:4);
  else
      fprintf('SPE right boundary physical number not found!');
  end 
%  corner nodes
