...
function [nnode,coords,connect,elemTag,bound] = read_input_file_speT3(infile) 
% 
%   clear;clc;
...
% 
  if ( string(phyname{3,2}) == '"matrix"' )
      connect_mat = connect((connect(:,1) == phyname{3,1}), 3:5); % SPE 
  else
      fprintf('SPE physical number matrix not found!');
  end
  if ( string(phyname{4,2}) == '"inclusion"' )
      connect_inc = connect((connect(:,1) == phyname{4,1}), 3:5); % SPE 
  else
      fprintf('SPE physical number inclusion not found!');
  end
%   
%%
%
...
%   if ( string(phyname{3,2}) == '"lower"' )
%       bound.lower = connect((connect(:,1) == phyname{3,1}),3:4);
%   else
%       fprintf('SPE left boundary physical number not found!');
%   end 
%   if ( string(phyname{4,2}) == '"upper"' )
%       bound.upper = connect((connect(:,1) == phyname{4,1}),3:4);
%   else
%       fprintf('SPE right boundary physical number not found!');
%   end 
%%
