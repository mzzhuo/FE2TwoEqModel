...
% figure;
hold on;
plot(T_mat(:,1),T_mat(:,2),'-','Linewidth',3,'MarkerSize',3)
...
% figure;
hold on;
plot(T_inc(:,1), T_inc(:,2),'r-o','Linewidth',2,'MarkerSize',3)


%% write to files
data = T_mat;
fileID = fopen('eg3macroT_mat_72s_new.txt','w');
fprintf(fileID,'%12.6e, %12.6e \n',data');
fclose(fileID);

data = T_inc;
fileID = fopen('eg3macroT_inc_72s_new.txt','w');
fprintf(fileID,'%12.6e, %12.6e \n',data');
fclose(fileID);
